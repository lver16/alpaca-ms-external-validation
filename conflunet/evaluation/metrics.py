...
# Put the absolute path to the folder that contains "conflunet"
PROJECT_ROOT = "/conflunet/evaluation"   # <-- CHANGE THIS
...
def compute_metrics(
    ...
    ####################################################################################################
    if precomputed_cl_ids is not None:
        cl_ids_tier0_conn6  = precomputed_cl_ids.get((0, 6),  [])
        cl_ids_tier0_conn26 = precomputed_cl_ids.get((0, 26), [])
        cl_ids_tier1_conn6  = precomputed_cl_ids.get((1, 6),  [])
        cl_ids_tier1_conn26 = precomputed_cl_ids.get((1, 26), [])
    else:
        cl_ids_tier0_conn6  = find_confluent_lesions(instance_ref)
        cl_ids_tier0_conn26 = find_confluent_lesions(instance_ref, connectivity=26)
        cl_ids_tier1_conn6  = find_tierx_confluent_instances(instance_ref, tier=1)
